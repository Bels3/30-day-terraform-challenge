# 30-Day Terraform Challenge

My journey through the 30-Day Terraform Challenge, building real infrastructure with Terraform and AWS.

## Progress

| Day | Topic | Status |
|-----|-------|--------|
| Day 01 | Introduction to Terraform and IaC | Done |
| Day 02 | Setting Up Terraform Environment | Done |
| Day 03 | Deploying Your First Server | Done |
| Day 04 | Mastering Basic Infrastructure | Done |
| Day 05 | Scaling Infrastructure and Terraform State | Done |
| Day 06 | Understanding and Managing Terraform State | Done |
| Day 07 | State Isolation — Workspaces vs File Layouts | Done |
| Day 08 | Building Reusable Infrastructure with Terraform Modules |
| Day 09 | Module Versioning with GitHub Tags | Done |
| Day 10 | Loops and Conditionals in Terraform | Done |
| Day 11 | Advanced Terraform Patterns | Done |
| Day 12 | Zero-Downtime Deployments with Terraform | Done |
| Day 13 | | Upcoming |



## Communities
- AWS AI/ML UserGroup Kenya
- Meru HashiCorp User Group
- EveOps

## Tools
- Terraform v1.14.7
- AWS CLI v2.33.1
- Ubuntu 24
- Region: eu-west-1

## Remote State
- S3 Bucket: terraform-state-beldine-2026
- DynamoDB Table: terraform-state-locks
