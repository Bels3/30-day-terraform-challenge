...
resource "aws_db_instance" "main" {
  ...
  # Add this for cross-region replica encryption
  kms_key_id = var.is_replica && var.replicate_source_db != null ? aws_kms_key.replica[0].arn : null
...
# Add KMS key for replica
resource "aws_kms_key" "replica" {
  count       = var.is_replica ? 1 : 0
  description = "KMS key for RDS replica ${var.identifier}"
  tags        = local.common_tags
}
